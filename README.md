...
- `ArrayBlockingQueue`：有界队列，基于数组的先进先出（FIFO），其构造必须指定大小

- `LinkedBlockingQueue`：无界队列，因为数量最大限制是 `Integer.MAX_VALUE` 所以被称为无界。基于链表的先进先出（FIFO）

- `SynchronizedQueue`：

- `PriorityBlockingQueue`：带优先级的无界队列，每次出队都返回优先级最高或者最低的元素(这里规则可以自己Comparable)，内部是使用平衡二叉树实现的，遍历不保证有序
...
- **synchronized 依赖于 JVM 而 ReentrantLock 依赖于 API**
  ...
  - `synchronized` 是虚拟机层面实现的，并没有直接暴露给我们，会自动的获取和释放锁。
...
- **可见性** ：当一个变量对共享变量进行了修改，那么另外的线程都是立即可以看到修改后的最新值。synchronized和volatile关键字可以保证共享变量的可见性

- **有序性** ：`volatile` 关键字可以禁止指令进行重排序优化。

**volatile 关键字能保证数据的可见性、变量可见性，但不能保证数据的原子性。synchronized 关键字保证代码片段的原子性和变量可见性**
