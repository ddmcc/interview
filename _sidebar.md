- Java基础
  - [String类和常量池](?id=String类和常量池)
    ...
    - [String、StringBuffer、StringBuilder的区别](?id=String、StringBuffer、StringBuilder的区别)
    - [为什么Java9后使用byte字节数组而舍弃了char字符数组](?id=为什么Java9后使用byte字节数组而舍弃了char字符数组)
...
- 框架
  ...
  - Mybatis
    ...
    - [MyBatis是如何进行分页的？分页插件的原理是什么？](?id=MyBatis是如何进行分页的？分页插件的原理是什么？)
    - [MyBatis的插件运行原理以及如何编写一个插件？](?id=MyBatis的插件运行原理以及如何编写一个插件？)
...
- 数据库
  ...
  - [怎么给线上的大表添加索引？](?id=怎么给线上的大表添加索引？)
  - [说说Double-Write（解决什么问题，工作流程）](?id=说说Double-Write（解决什么问题，工作流程）)
  - [RR级别下会产生幻读吗？InnoDB是怎么解决的？](?id=RR级别下会产生幻读吗？InnoDB是怎么解决的？)
...
- redis
  ...
  - [Redis6.0之后为何引入了多线程？](?id=Redis6.0之后为何引入了多线程？)
  - [为什么要用Redis/为什么要用缓存？](?id=为什么要用Redis、为什么要用缓存？)
  - [Redis是如何判断数据是否过期的呢？](?id=Redis是如何判断数据是否过期的呢？)
  - [过期的数据的删除策略了解么？](?id=过期的数据的删除策略了解么？)
  - [Redis持久化机制](?id=Redis持久化机制)
  - [Redis事务](?id=Redis事务)
  - [什么是缓存穿透？](?id=什么是缓存穿透？)
  - [什么是缓存雪崩？](?id=什么是缓存雪崩？)
  - [缓存一致性](?id=缓存一致性)
